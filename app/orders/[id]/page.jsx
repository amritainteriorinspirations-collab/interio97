...
import {
  ...
  CheckCircle, Clock, ArrowLeft, Download, Tag,
  Phone,
} from "lucide-react";
...
export default function OrderDetailsPage() {
  ...
  // Canonical totals: { mrp, discount, subtotal, deliveryCharge, grandTotal }
  ...
  const paymentStatusIcons = { pending: Clock, paid: CheckCircle, failed: Package, not_required: CheckCircle };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <button onClick={() => router.back()} className="flex items-center gap-2 text-blue-600 hover:text-blue-700 mb-6 font-medium">
        ...
        <div className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden mb-6">
          ...
          <div className="px-6 py-6 border-b border-gray-200">
            ...
            <div className="flex items-center gap-3">
              <span className={`px-4 py-2 rounded-full text-sm font-semibold ${statusColors[order.orderStatus]}`}>
                {order.orderStatus.replace(/_/g, " ").toUpperCase()}
              </span>
              {order.paymentStatus !== "not_required" && (
                <div className="flex items-center gap-2">
                  <PaymentStatusIcon size={18} className="text-gray-600" />
                  <span className="text-sm text-gray-600">Payment: {order.paymentStatus.toUpperCase()}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="px-6 py-6">
            <div className="grid grid-cols-3 gap-6">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">₹{fmt(grandTotal)}</p>
              </div>
              <div>
                  ...
                  return (
                    <div key={item.productId} className="flex gap-4 pb-5 border-b border-gray-100 last:border-0 last:pb-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-900 leading-snug">{item.productSnapshot.name}</p>
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          {hasDiscount && <span className="text-xs text-gray-400 line-through">₹{fmt(p.unitOriginalPrice)}</span>}
                          <span className="text-xs text-gray-500">/ {item.sellBy}</span>
                        </div>
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {hasDiscount && (
                            <span className="inline-flex items-center gap-1 bg-green-50 text-green-700 text-xs font-semibold px-2 py-0.5 rounded-full border border-green-200">
                              <Tag size={10} /> {p.discountPercent}% OFF
                            </span>
                          )}
                        </div>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="font-bold text-gray-900">₹{fmt(p.lineFinalTotal)}</p>
                        {hasDiscount && (
                          <>
              ...
              <div className="p-6 space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">
                    Market Price <span className="text-gray-400 text-xs">(before discounts)</span>
                  </span>
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 flex items-center gap-1">
                    <Truck size={14} className="text-gray-400" /> Delivery Charge
                  </span>
                ...
                <div className="border-t-2 border-gray-200 pt-4 flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <span className="text-2xl font-bold text-orange-600">₹{fmt(grandTotal)}</span>
                    {discount > 0 && (
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
              ...
              <div className="p-6 space-y-1.5">
                <p className="font-semibold text-gray-900">{order.addressSnapshot.name}</p>
                <p className="text-sm text-gray-600">{order.addressSnapshot.addressLine1}</p>
                {order.addressSnapshot.addressLine2 && (
                  <p className="text-sm text-gray-600">{order.addressSnapshot.addressLine2}</p>
                )}
                <p className="text-sm text-gray-600">
                ...
                <p className="text-sm text-gray-600 flex items-center gap-2 pt-2">
                  <Phone size={16} className="text-gray-600" /> {order.addressSnapshot.phone}
                </p>
              </div>
            </div>
              ...
              <div className="p-6 space-y-3">
                ...
                {(order.orderStatus === "shipped" || order.orderStatus === "delivered") && (
                  <div className="flex gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 text-sm">On the Way</p>
                      <p className="text-xs text-gray-600 mt-0.5">Est. 3-5 business days</p>
                    </div>
                  </div>
                )}
                {order.orderStatus === "delivered" && (
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600 mt-0.5">Thank you for your purchase!</p>
                    </div>
                  </div>
                )}
              </div>
