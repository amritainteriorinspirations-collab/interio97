...
export default function CategoryPageClient({ products, userRole, headerContent }) {
  ...
  const handleFilterChange = (filters, sortBy) => {
    ...
    if (filters.sizes.length) result = result.filter((p) => filters.sizes.includes(p.size));
    ...
    if (filters.materials?.length)
      result = result.filter((p) => p.material?.some((m) => filters.materials.includes(m)));

    if (filters.patterns?.length)
      result = result.filter((p) => p.pattern?.some((m) => filters.patterns.includes(m)));

    if (filters.finishes?.length)
      result = result.filter((p) => p.finish?.some((f) => filters.finishes.includes(f)));

    // FIXED: application filtering uses slug
    if (filters.applications?.length)
      result = result.filter((p) => p.application?.some((a) => filters.applications.includes(a.slug)));
...
<div className={`${showFilters ? "block" : "hidden"} sm:block sm:sticky sm:top-24`}>
          <FilterSidebar products={products} onFilterChange={handleFilterChange} userRole={userRole} />
        </div>
      </aside>
        ...
        {filteredProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-5">
            {filteredProducts.map((product, index) => (
              <ProductCardGrid key={product.id || index} product={product} userRole={userRole} />
            ))}
          </div>
        )}
      </main>
