...
const sellBy = product.sellBy ?? "unit";
  ...
  return (
    <Link href={`/product/${product.slug}`} className="block">
      <article className="group bg-white rounded-lg border border-gray-200 hover:shadow-lg transition-shadow duration-250 overflow-hidden h-full flex flex-col">
...
<div className="mt-auto">
    ...
    <div className="flex flex-col leading-tight">
      ...
      <span className="text-[13px] font-semibold text-gray-900">
        ...
        {product.showPerSqFtPrice ? (
          ...
          <span className="text-[10px] text-gray-500"> / {sellBy}</span>
        )}
      </span>
      ...
      {secondaryPrice?.savings > 0 && (
        <span className="text-[10px] text-green-700 font-medium">
          Save ₹{Number(secondaryPrice.savings).toLocaleString("en-IN")}
        </span>
      )}
    </div>
    ...
    <div className="flex flex-col items-end leading-tight">
      ...
      {product.showPerSqFtPrice && (
        <span className="text-[12px] text-gray-900 font-medium">
          ₹{Number(secondaryPrice.discounted).toLocaleString("en-IN")} / {sellBy}
        </span>
      )}
  ...
  {secondaryPrice?.discountPercent > 0 && (
    <div className="mt-1 w-fit ml-auto bg-green-600 text-white rounded-md px-2 py-[2px] text-[10px] font-semibold">
