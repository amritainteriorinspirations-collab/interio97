...
export default function ProductDetails({ product }) {
  const visibleAttributes = buildAttributes(product).filter((a) => a.show);
  const hasApplications   = product.application?.length > 0;

  if (!product.description && visibleAttributes.length === 0 && !hasApplications) {
    return null;
  }

  return (
    <div className="mb-4">
      <div className="bg-white rounded-sm border border-gray-200 p-4 sm:p-5 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          ...
          {/* Divider between description and specs */}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                {visibleAttributes.map((attr, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="bg-gray-50 border border-gray-200 rounded-sm p-2.5 flex items-start gap-2"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[12px] text-gray-900 capitalize font-medium break-words mt-0.5">
                          {attr.value}
                        </p>
                      </div>
                    </div>
          ...
          {/* ── Applications (divider rendered inside ApplicationsGallery) ── */}
          {hasApplications && (
